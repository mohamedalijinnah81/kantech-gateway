Imports System.IO
Namespace KantechGatewayApp.Infrastructure
    Public NotInheritable Class Logger
        Private Shared ReadOnly logRoot As String = IO.Path.GetFullPath(System.Configuration.ConfigurationManager.AppSettings("Root.Logs"))

        Private Shared Function LogPath() As String
            Dim fname = If(Boolean.Parse(System.Configuration.ConfigurationManager.AppSettings("Log.RollDaily")), $"app_{Date.Today:yyyyMMdd}.log", "app.log")
            Directory.CreateDirectory(logRoot)
            Return Path.Combine(logRoot, fname)
        End Function

        Public Shared Sub Info(msg As String)
            Write("INFO", msg)
        End Sub
        Public Shared Sub Warn(msg As String)
            Write("WARN", msg)
        End Sub
        Public Shared Sub [Error](msg As String, Optional ex As Exception = Nothing)
            Write("ERROR", If(ex Is Nothing, msg, $"{msg} :: {ex}"))
        End Sub

        Private Shared Sub Write(level As String, msg As String)
            Dim line = $"{DateTime.Now:yyyy-MM-dd HH:mm:ss} [{level}] {msg}"
            SyncLock GetType(Logger)
                File.AppendAllText(LogPath(), line & Environment.NewLine)
            End SyncLock
        End Sub
    End Class
End Namespace